...
export const blueStackBehavior: RelictBehavior = {
  onCanPlaceTile: (tile: Tile, position: BoardPosition, board: (Tile | null)[][], isFirstTile: boolean) => {
    ...
    if (tile.matchesColor('blue')) {
      ...
      if (targetTile && targetTile.matchesColor('red')) {
  ...
  onBeforeTilePlacement: (context: TilePlacementContext): TilePlacementResult => {
    ...
    // If placing a blue tile on a red tile, add consume effect
    if (context.tile.matchesColor('blue') && targetTile && targetTile.matchesColor('red')) {
      effects.push({
        type: 'consume' as RelictEffectType,
        relictId: 'blue-stack',
...
export const blueStackRelict: Relict = {
  ...
  name: 'Blue Override',
  icon: '🔵',
  description: '**Blue tiles** can be placed on **red tiles**. The new tile **consumes** the red tile below.',
