...
export const alchemyBehavior: RelictBehavior = {
  onBeforeTilePlacement: (context: TilePlacementContext) => {
    	if (context.tile.matchesColor('green')) {
      return { tile: { ...context.tile, color: 'red', number: context.tile.number * 2 }, canPlace: true, board: context.board };
    }
    return { tile: context.tile, canPlace: true, board: context.board };
  },
};
